    ...
    def _generate_recommendations(
        ...
        if analysis.get("total_lines", 0) > 500:
            recommendations.append(
                "📊 Projeto grande detectado. Considere modularizar ainda mais."
            )
        ...
        if 'numpy' in imports_str.lower():
            recommendations.append(
                "⚡ Use np.vectorize() e broadcasting para otimizar operações"
            )
        ...
        if 'matplotlib' in imports_str.lower():
            recommendations.append(
                "🎨 Considere Plotly para gráficos interativos"
            )
        ...
        if research.get("results"):
            ...
            recommendations.append(
                f"📚 Leia: {best_paper['title']} - {best_paper['url']}"
            )
...
Exemplos:
  ...
  %(prog)s --research "machine learning for power systems"
    ...
    parser.add_argument(
        '--project', 
        type=Path,
        help='Caminho do projeto para analisar'
    )
    
    parser.add_argument(
        '--analyze-pd',
        action='store_true',
        help='Analisa projeto de partial discharge'
    )
    
    parser.add_argument(
        '--research',
        type=str,
        help='Tópico para pesquisar'
    )
